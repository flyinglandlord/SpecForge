...
def initialize_tree(input_ids, model, past_key_values, logits_processor, unk_head_softmax=False):
    ...
    draft_tokens, retrieve_indices,tree_mask,tree_position_ids,logits = model.ea_layer.topK_genrate(
        hidden_states, input_ids, model.base_model.lm_head, logits_processor, unk_head_softmax=unk_head_softmax)
    return draft_tokens, retrieve_indices,tree_mask,tree_position_ids, orig, hidden_states, token, logits
...
def update_inference_inputs(
    ...
    draft_tokens, retrieve_indices,tree_mask,tree_position_ids, logits = model.ea_layer.topK_genrate(accept_hidden_state_new,
                                              input_ids=torch.cat((input_ids, token.to(input_ids.device)), dim=1),
                                              head=model.base_model.lm_head, logits_processor=logits_processor, 
                                              unk_head_softmax=unk_head_softmax)


    new_token += accept_length + 1

    return input_ids, draft_tokens, retrieve_indices,tree_mask,tree_position_ids, new_token, None, token, logits
